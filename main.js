  ...
  async handleApi(request, url, env, ctx) {
    ...
    if (url.pathname === "/api/update-panel" && request.method === "POST") {
      if (!env.CF_API_TOKEN || !env.CF_ACCOUNT_ID) {
        return new Response(
          JSON.stringify({ error: "CF_API_TOKEN or CF_ACCOUNT_ID not set." }),
      ...
      try {
        ...
        if (!bindingsData.success)
          throw new Error("Unable to access worker settings. Invalid token.");
        const newBindings = [];
        ...
        const errorMsg =
          err.message +
          " | If unsuccessful, update via: https://ryxo-deployer.sepantagamer724.workers.dev";
...
const SubscriptionService = {
  async generateJson(user, host, env) {
    ...
    const m2 = decodeURIComponent(
      "%E2%99%A8%EF%B8%8F%20%40RyxoPanel%20%D8%B3%D8%A7%D8%AE%D8%AA%20%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%20%E2%99%A8%EF%B8%8F",
    );
...
// ==========================================================
// 9. HTML TEMPLATES (Translated to English)
// ==========================================================
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-8 text-center flex flex-col items-center gap-4">
        ...
        <h2 class="text-xl font-bold text-gray-900 dark:text-white">Admin Panel Access</h2>
            ...
            <table class="w-full text-center border-collapse">
                <thead>
                    <tr class="bg-gray-100 dark:bg-zinc-900/50 border-b border-gray-200 dark:border-amoled-border text-xs text-gray-500 dark:text-gray-400">
                        <th class="p-4 border-r border-gray-200 dark:border-zinc-800">User & Actions</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Subscription Link</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Protocol</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Port</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Usage Status</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Request Status</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Expiry Status</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Online Users</th>
                        <th class="p-2 border-r border-gray-200 dark:border-zinc-800">Created</th>
                    </tr>
                </thead>
...
<div id="path-warning-modal" class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-red-500/50 rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-2">Important Configuration Change</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
            Due to security upgrades, configurations prior to version 1.3.4 are no longer valid. Please update your subscription if you are unable to connect.
        </p>
...
<div id="usage-warning-modal" class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-orange-500/50 rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-2">Daily Request Limit Warning</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
            Your Cloudflare daily requests have exceeded 90,000. If you exceed the free limit of 100,000, panel access and connections will be interrupted until 3:30 AM (Iran time).
        </p>
            ...
            <form id="create-user-form" class="p-6 space-y-5 overflow-y-auto flex-1 overscroll-contain" style="-webkit-overflow-scrolling: touch; transform: translate3d(0,0,0); will-change: scroll-position, transform;" onsubmit="handleFormSubmit(event)">
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div>
                            <label class="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Volume (GB)</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 right-0 flex items-center pr-3.5 pointer-events-none text-gray-400">
                        ...
                        <div>
                            <label class="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Request Limit</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 right-0 flex items-center pr-3.5 pointer-events-none text-gray-400">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                                </span>
                                <input type="number" id="input-req-limit" min="0" placeholder="Unlimited" class="w-full pl-3 pr-10 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                            </div>
                        </div>
                        <div>
                ...
                <div class="pt-2 border-t border-gray-100 dark:border-zinc-900">
                    <label class="block text-xs font-bold text-gray-500 dark:text-zinc-400 mb-3 uppercase tracking-wider">Connection Ports (Multi-select)</label>
                    <div class="space-y-4">
                        <div class="p-4 bg-gray-50/50 dark:bg-zinc-900/20 border border-gray-200/60 dark:border-zinc-850 rounded-2xl shadow-sm">
                            <div class="flex items-center gap-1.5 mb-3">
                                ...
                                <span class="text-xs font-bold text-blue-600 dark:text-blue-400">🔒 TLS Ports (Secure)</span>
                            </div>
                        ...
                        <div class="p-4 bg-gray-50/50 dark:bg-zinc-900/20 border border-gray-200/60 dark:border-zinc-850 rounded-2xl shadow-sm">
                            <div class="flex items-center gap-1.5 mb-3">
                                ...
                                <span class="text-xs font-bold text-amber-600 dark:text-amber-400">🔓 Non-TLS Ports (Standard)</span>
                            </div>
                ...
                <div class="pt-4 border-t border-gray-100 dark:border-zinc-900 space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="block text-xs font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Clean Cloudflare IPs (Optional)</label>
    ...
    <script>
        ...
        function toggleModal(show) {
            ...
            if (show) {
                ...
                // Reset ports 443 and 80 to default
                const cb443 = document.querySelector('input[name="ports"][value="443"]');
                if (cb443) cb443.checked = true;
                const cb80 = document.querySelector('input[name="ports"][value="80"]');
                if (cb80) cb80.checked = true;
                
                // Reset fingerprint to iOS
                ...
                tbody.innerHTML = users.map(user => {
                    ...
                    if (user.limit_req) {
                        ...
                        reqHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px] select-none">' +
                            ...
                            '</div>' +
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-left flex-1 whitespace-nowrap">' +
                    ...
                    if (user.max_connections) {
                        ...
                        onlineHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px] select-none">' +
                            ...
                            '</div>' +
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-left flex-1 whitespace-nowrap">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Connected: ' + onlineCount + '</span>' +
        ...
        function getVlessLink(username) {
            ...
            const m2 = decodeURIComponent('%E2%99%A8%EF%B8%8F%20%40RyxoPanel%20%D8%B3%D8%A7%D8%AE%D8%AA%20%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%20%E2%99%A8%EF%B8%8F');
        ...
        function copySubLink(encodedUsername) {
            ...
            navigator.clipboard.writeText(getSubLink(username)).then(() => {
                alert('✅ Text subscription link copied successfully!');
            }).catch(() => {
                alert('Failed to copy subscription link!');
            });
        }
        ...
        function copyStatusLink(encodedUsername) {
            ...
            navigator.clipboard.writeText(getStatusLink(username)).then(() => {
                alert('✅ Status page link copied successfully!');
            }).catch(() => {
                alert('Failed to copy status page link!');
            });
        }
        ...
        function copyJsonSubLink(encodedUsername) {
            ...
            navigator.clipboard.writeText(getJsonSubLink(username)).then(() => {
                alert('✅ JSON subscription link copied successfully!');
            }).catch(() => {
                alert('Failed to copy JSON subscription link!');
            });
        }
        ...
        function copyConfig(encodedUsername) {
            ...
            navigator.clipboard.writeText(link).then(() => {
                alert('✅ VLESS config copied successfully!');
            }).catch(() => {
        ...
        function copyJsonConfig(encodedUsername) {
            ...
            navigator.clipboard.writeText(JSON.stringify(configArray, null, 2)).then(() => {
                alert('✅ JSON config copied successfully!');
            }).catch(() => {
                alert('Failed to copy JSON config!');
            });
        }
        ...
        async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/itzsepanta/RyxoPanel/refs/heads/main/main.js?t=' + Date.now());
        ...
        async function applyUpdate() {
            ...
            try {
                ...
                if (res.ok && data.success) {
                    alert('Panel successfully updated to the latest version! Page will now refresh.');
                    window.location.reload();
                } else {
                ...
                alert('Server communication error for update.');
                btn.disabled = false;
            }
        }
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>User Subscription Status</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"></script>
    <link href="https://cdn.jsdelivr.net/gh/rastikerdar/vazirmatn@v33.003/Vazirmatn-font-face.css" rel="stylesheet" type="text/css" />
    <script>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">Subscription Status</h1>
            ...
            <div id="live-connections-badge" class="hidden inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-full text-xs font-bold shadow-sm">
                ...
                <span id="live-connections-text" dir="ltr">0 devices connected</span>
            </div>
        </div>
        ...
        <div class="space-y-5 mb-8 relative z-10">
            ...
            <div class="bg-white/40 dark:bg-zinc-900/30 border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm">
                ...
                <div class="flex justify-between text-xs text-gray-500 dark:text-zinc-400 font-medium">
                    ...
                    <span>Total: <span id="limit-req" class="font-bold text-gray-800 dark:text-zinc-200">-</span></span>
                </div>
            </div>
            ...
            <div class="bg-white/40 dark:bg-zinc-900/30 border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm">
                ...
                <div class="flex justify-between text-xs text-gray-500 dark:text-zinc-400 font-medium">
                    <span>Connected: <span id="online-count" class="font-bold text-gray-800 dark:text-zinc-200">0</span></span>
        ...
        <div class="border-t border-gray-100 dark:border-zinc-800 pt-6 relative z-10">
            ...
            <div class="space-y-3">
                ...
                <button onclick="copyVlessConfig()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-blue-500 dark:hover:border-blue-500 rounded-xl text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2">🚀 Copy VLESS Config (Direct)</span>
                ...
                <button onclick="showQR()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-emerald-500 dark:hover:border-emerald-500 rounded-xl text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2">📱 Show QR Code</span>
                    <span class="text-emerald-500">View</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function copyVlessConfig() {
            navigator.clipboard.writeText(getVlessLink()).then(() => alert('✅ VLESS config copied successfully!'));
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (u.is_active === 0) {
                ...
                statusText.innerText = '❌ Subscription Status: Disabled / Manually Blocked';
                ...
                statusText.innerText = '⚠️ Subscription Status: Data Limit Reached';
            } else if (isReqExpired) {
                statusCard.className = 'mb-6 rounded-2xl p-4 text-center border font-bold relative z-10 bg-yellow-500/10 border-yellow-500/30 text-yellow-500 shadow-md shadow-yellow-500/5';
                statusText.innerText = '⚠️ Subscription Status: Request Limit Reached';
            } else if (isTimeExpired) {
                statusCard.className = 'mb-6 rounded-2xl p-4 text-center border font-bold relative z-10 bg-yellow-500/10 border-yellow-500/30 text-yellow-500 shadow-md shadow-yellow-500/5';
                statusText.innerText = '⏳ Subscription Status: Expired';
            } else {
                ...
                statusText.innerText = '✅ Subscription Status: Active & Connected';
            }
        });
    </script>
</body>
