...
// ==========================================================
const CURRENT_VERSION = "26.2.1";
const PANEL_NAME = "Ryxo";
const REPO_URL = "https://github.com/itzsepanta/RyxoPanel";
const DEPLOYER_URL = "https://ryxo-deployer.sepantagamer724.workers.dev";
const PROJECT_SITE = "itzsepanta.github.io/RyxoPanel/";
  ...
  async handleApi(request, url, env, ctx) {
    ...
    if (url.pathname === "/api/update-panel" && request.method === "POST") {
      if (!env.CF_API_TOKEN || !env.CF_ACCOUNT_ID) {
        return new Response(
          JSON.stringify({ error: "Cloudflare token or account ID not set." }),
      ...
      try {
        ...
        if (!githubRes.ok)
          throw new Error("Failed to fetch new source from GitHub");
        ...
        if (!bindingsData.success)
          throw new Error("Cannot access worker bindings. Token invalid.");
        const newBindings = [];
        ...
        const errorMsg =
          err.message + " | If unsuccessful, update via: " + DEPLOYER_URL;
...
const SubscriptionService = {
  async generateJson(user, host, env) {
    ...
    const m1 = "⚠️ This panel is free and not for sale ⚠️";
    const m2 = "♨️ @itzsepanta - Free Ryxo Panel ♨️";
...
async function flushExpiredTraffic(env) {
  ...
  for (const [uname, cachedBytes] of GLOBAL_TRAFFIC_CACHE.entries()) {
    ...
    if (activeCount <= 0 || now - lastActive > 65000) {
      ...
      try {
        await env.DB.prepare(
          "UPDATE users SET used_gb = used_gb + ?, used_req = used_req + ? WHERE username = ?",
        )
          .bind(deltaGb, cachedReqs, uname)
          .run();
      } catch (e) {
      } finally {
...
async function handleVLESS(env, storedData = null, ctx = null) {
  ...
  const processWsMessage = async (chunk) => {
    ...
    if (!isHeaderParsed) {
      ...
      if (
        user.limit_req &&
        user.used_req + (USER_REQ_CACHE.get(user.username) || 0) >=
          user.limit_req
      ) {
        serverSock.close();
        return;
      ...
      if (
        ...
        activeCount >= user.max_connections
      ) {
        serverSock.close();
        return;
      }
...
// ==========================================================
// NOTE: HTML Templates are included but shortened for brevity.
// The full panel HTML is over 2000 lines and contains the complete Ryxo Panel UI.
// All references to "Zeus" have been changed to "Ryxo" and version to "26.2.1".
// ==========================================================
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-8 text-center flex flex-col items-center gap-4">
        ...
        <h2 class="text-xl font-bold text-gray-900 dark:text-white">Ryxo Panel Access</h2>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ryxo Panel - Setup Password</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:opsz@14..32&display=swap" rel="stylesheet">
    <script>
        tailwind.config = {
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-6">
        <h2 class="text-xl font-bold mb-2 text-center text-blue-600 dark:text-blue-400">Set Panel Password</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 text-center mb-6">This is your first time accessing the panel. Please set a password.</p>
        <form onsubmit="handleSetup(event)" class="space-y-4">
            ...
            <button type="submit" id="submit-btn" class="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg text-sm transition font-bold">Save & Login</button>
        </form>
    </div>
    <script>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-6">
        <h2 class="text-xl font-bold mb-2 text-center text-blue-600 dark:text-blue-400">Ryxo Panel Login</h2>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ryxo Panel v26.2.1</title>
    <script>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen transition-colors duration-200">
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg font-bold flex items-center gap-2" dir="ltr">
                    Ryxo Panel
                    <span id="panel-version" class="text-xs px-2 py-0.5 font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 rounded-full">v26.2.1</span>
                </h1>
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://github.com/itzsepanta/RyxoPanel" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="GitHub">
                    ...
                    <a href="https://t.me/itzsepanta" target="_blank" rel="noopener noreferrer" class="text-sky-500 hover:text-sky-600 dark:hover:text-sky-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Telegram">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.94-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.37.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .24z"/>
                        </svg>
                    </a>
                    <a href="https://ryxo-deployer.sepantagamer724.workers.dev" target="_blank" rel="noopener noreferrer" class="text-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Deployer">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                    </a>
                </div>
            </div>
    ...
    <main class="max-w-6xl mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
            <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm stat-card relative overflow-hidden">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-semibold text-gray-500 dark:text-zinc-400">Total Users</span>
                ...
                <div class="text-2xl font-black text-gray-900 dark:text-zinc-100" id="stat-total-users">0</div>
                <span class="text-[11px] text-indigo-500 dark:text-indigo-400 font-medium">All users</span>
            </div>

            <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm stat-card relative overflow-hidden">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-semibold text-gray-500 dark:text-zinc-400">Online Now</span>
                ...
                <div class="text-2xl font-black text-emerald-600 dark:text-emerald-400" id="stat-active-users">0</div>
                <span class="text-[11px] text-emerald-500 dark:text-emerald-400 font-medium flex items-center gap-1">
                    <span class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span> Active connections
                </span>
            </div>

            <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm stat-card relative overflow-hidden">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-semibold text-gray-500 dark:text-zinc-400">Daily Requests</span>
                ...
                <div class="flex items-center gap-1">
                    <span class="text-2xl font-black text-orange-600 dark:text-orange-400" id="stat-cf-requests">0</span>
                ...
                <span class="text-[11px] text-orange-500 dark:text-orange-400 font-medium">Total: <span id="stat-cf-total">0</span></span>
            </div>

            <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm stat-card relative overflow-hidden">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-semibold text-gray-500 dark:text-zinc-400">Total Traffic</span>
                ...
                <div class="text-2xl font-black text-blue-600 dark:text-blue-400" id="stat-total-usage">0 GB</div>
                <span class="text-[11px] text-blue-500 dark:text-blue-400 font-medium">Total usage (30 days)</span>
            </div>

            <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm stat-card relative overflow-hidden">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-semibold text-gray-500 dark:text-zinc-400">Top User</span>
                ...
                <div class="text-xl font-black text-amber-600 dark:text-amber-400 truncate" id="stat-top-user">-</div>
                <span class="text-[11px] text-amber-500 dark:text-amber-400 font-medium" id="stat-top-user-usage">0 GB used</span>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-100 dark:bg-zinc-900/50 border-b border-gray-200 dark:border-amoled-border text-xs text-gray-500 dark:text-gray-400 text-center">
                        ...
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Subscription</th>
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Protocol</th>
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Ports</th>
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Traffic</th>
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Requests</th>
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Expiry</th>
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Online</th>
                        <th class="p-2 border-l border-gray-200 dark:border-zinc-800">Created</th>
                    </tr>
                </thead>
        ...
        <div id="empty-state" class="hidden p-8 border border-dashed border-gray-300 dark:border-amoled-border rounded-2xl text-center">
            <p class="text-gray-500 dark:text-gray-400">No users found. Click + to create one.</p>
        </div>
    </main>
    ...
    <div id="user-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
        <div id="user-modal-card" class="w-full max-w-xl bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-850 rounded-2xl shadow-xl overflow-hidden transition-[opacity,transform] duration-200 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh] transform-gpu">
            ...
            <form id="create-user-form" class="p-6 space-y-5 overflow-y-auto flex-1" onsubmit="handleFormSubmit(event)">
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Username</label>
                        <input type="text" id="input-name" placeholder="username" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm font-semibold text-gray-800 dark:text-zinc-100" required>
                    </div>
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="number" id="input-limit" min="0" step="any" placeholder="Unlimited" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm font-semibold text-gray-800 dark:text-zinc-100">
                        </div>
                        <div>
                            ...
                            <input type="number" id="input-expiry" min="0" placeholder="Unlimited" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm font-semibold text-gray-800 dark:text-zinc-100">
                        </div>
                        <div>
                            <label class="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Req Limit</label>
                            <input type="number" id="input-req-limit" min="0" placeholder="Unlimited" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm font-semibold text-gray-800 dark:text-zinc-100">
                        </div>
                        <div>
                            ...
                            <input type="number" id="input-max-connections" min="0" placeholder="Unlimited" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm font-semibold text-gray-800 dark:text-zinc-100">
                        </div>
                    </div>
                </div>

                <div class="pt-2 border-t border-gray-100 dark:border-zinc-900">
                    <label class="block text-xs font-bold text-gray-500 dark:text-zinc-400 mb-3 uppercase tracking-wider">Ports</label>
                    <div class="space-y-4">
                        <div class="p-4 bg-gray-50/50 dark:bg-zinc-900/20 border border-gray-200/60 dark:border-zinc-850 rounded-2xl">
                ...
                <div class="pt-4 border-t border-gray-100 dark:border-zinc-900 space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Clean IPs (Optional)</label>
                        <textarea id="input-ips" rows="2" placeholder="104.16.0.1" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-mono text-gray-800 dark:text-zinc-100 resize-none"></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Fingerprint</label>
                        <select id="fingerprint-select" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-semibold text-gray-700 dark:text-zinc-300 cursor-pointer">
                ...
                <div class="pt-4 flex gap-3">
                    ...
                    <button type="submit" id="submit-btn" class="flex-1 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl text-sm transition duration-200 shadow-md">Create User</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="settings-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium mb-1.5 text-gray-700 dark:text-zinc-300">Proxy Location</label>
                    <select id="location-select" class="w-full px-3 py-2.5 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-zinc-200 cursor-pointer">
                ...
                <div class="pt-4 border-t border-gray-100 dark:border-zinc-800">
                    <h4 class="text-sm font-bold mb-3 text-gray-800 dark:text-zinc-200">🔒 Change Password</h4>
                    <div class="space-y-3">
                        <div>
    ...
    <script>
        ...
        function renderPortCheckboxes() {
            ...
            tlsContainer.innerHTML = tlsPorts.map(function(port) {
                ...
                return '<label class="relative cursor-pointer">' +
                    '<input type="checkbox" name="ports" value="' + port + '" ' + checked + ' class="peer sr-only">' +
        ...
        function renderLocationsUI(locations, activeIata) {
            ...
            let html = '<option value="">🌐 Default (Auto)</option>';
        ...
        async function saveSettings() {
            ...
            try {
                ...
                if (response.ok) {
                    alert('✅ Settings saved successfully!\\n' + (iata ? 'Proxy IP: ' + resolvedIp : 'Proxy reset to default.'));
        ...
        async function changeAdminPassword() {
            ...
            if (!currentPwd || !newPwd) {
                alert('⚠️ Current and new password are required!');
                return;
            }
            if (newPwd.length < 4) {
        ...
        async function loadUsers(silent = false) {
            ...
            try {
                ...
                if (!silent) {
                    loadingState.innerHTML = '<span class="text-red-500">Error loading users</span>';
                }
            }
        }
        ...
        function renderFilteredUsers(users, serverTime) {
            ...
            if (users.length === 0) {
                ...
                if (allUsers.length > 0) {
                    emptyState.querySelector('p').innerText = 'No users matching search criteria.';
                } else {
                    emptyState.querySelector('p').innerText = 'No users yet. Click + to create one.';
                }
            } else {
                loadingState.classList.add('hidden');
                emptyState.classList.add('hidden');
                ...
                tbody.innerHTML = users.map(user => {
                    ...
                    if (user.limit_req) {
                        ...
                        reqHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                            ...
                            '</div>' +
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-right flex-1 whitespace-nowrap" dir="ltr">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Used: ' + usedReq.toLocaleString() + '</span>' +
                                '<span class="leading-none">Total: ' + user.limit_req.toLocaleString() + '</span>' +
                            '</div>' +
                        '</div>';
                    } else {
                        reqHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                            ...
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-right flex-1 whitespace-nowrap" dir="ltr">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Used: ' + usedReq.toLocaleString() + '</span>' +
                                '<span class="leading-none">Total: Unlimited</span>' +
                            '</div>' +
                        '</div>';
                    }
                    ...
                    if (user.limit_gb) {
                        ...
                        volumeHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                            '<div class="w-2 h-20 bg-gray-200 dark:bg-zinc-700 rounded-full flex flex-col justify-end overflow-hidden flex-shrink-0">' +
                            ...
                            '</div>' +
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-right flex-1 whitespace-nowrap" dir="ltr">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Used: ' + formattedUsed + '</span>' +
                                '<span class="leading-none">Total: ' + formattedLimit + '</span>' +
                            '</div>' +
                        '</div>';
                    } else {
                        volumeHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                    ...
                    if (user.expiry_days) {
                        ...
                        expiryHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                            '<div class="w-2 h-20 bg-gray-200 dark:bg-zinc-700 rounded-full flex flex-col justify-end overflow-hidden flex-shrink-0">' +
                            ...
                            '</div>' +
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-right flex-1 whitespace-nowrap" dir="ltr">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Remaining: ' + daysRemaining + ' days</span>' +
                                '<span class="leading-none">Total: ' + user.expiry_days + ' days</span>' +
                            '</div>' +
                        '</div>';
                    } else {
                        expiryHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                            ...
                            '</div>' +
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-right flex-1 whitespace-nowrap" dir="ltr">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Remaining: Unlimited</span>' +
                                '<span class="leading-none">Total: Unlimited</span>' +
                            '</div>' +
                        '</div>';
                    }
                    ...
                    if (user.max_connections) {
                        ...
                        onlineHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                            '<div class="w-2 h-20 bg-gray-200 dark:bg-zinc-700 rounded-full flex flex-col justify-end overflow-hidden flex-shrink-0">' +
                            ...
                            '</div>' +
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-right flex-1 whitespace-nowrap" dir="ltr">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Online: ' + onlineCount + '</span>' +
                                '<span class="leading-none">Max: ' + user.max_connections + '</span>' +
                            '</div>' +
                        '</div>';
                    } else {
                        onlineHtml = '<div class="flex flex-row items-center gap-2 w-full min-w-[90px]">' +
                            ...
                            '<div class="flex flex-col justify-between h-20 text-[10px] text-gray-500 dark:text-gray-400 font-medium text-right flex-1 whitespace-nowrap" dir="ltr">' +
                                '<span class="text-gray-800 dark:text-zinc-200 leading-none">Online: ' + onlineCount + '</span>' +
                                '<span class="leading-none">Max: Unlimited</span>' +
                            '</div>' +
                        '</div>';
                    }
                    ...
                    return '<tr class="hover:bg-gray-50 dark:hover:bg-zinc-900/40 border-b border-gray-100 dark:border-zinc-800 last:border-0">' +
                        '<td class="p-2 border-r border-gray-100 dark:border-zinc-800 text-center">' +
                            '<div class="flex flex-col items-center gap-1.5 w-[140px] mx-auto">' +
                                ...
                                '<div class="flex gap-1 w-full justify-center">' +
                                    (user.is_active === 0 ? '<span class="px-1.5 py-0.5 text-[10px] font-medium bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 rounded-md">Inactive</span>' : '<span class="px-1.5 py-0.5 text-[10px] font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 rounded-md">Active</span>') +
                                    (user.is_online === 1 ? '<span class="px-1.5 py-0.5 text-[10px] font-medium bg-emerald-500 text-white rounded-md animate-pulse">● Online (' + (user.online_count || 0) + (user.max_connections ? '/' + user.max_connections : '') + ')</span>' : '<span class="px-1.5 py-0.5 text-[10px] font-medium bg-gray-200 text-gray-600 dark:bg-zinc-800 dark:text-zinc-400 rounded-md">Offline</span>') +
                                '</div>' +
                                '<div class="grid grid-cols-3 gap-1 w-full">' +
                                    '<button onclick="copyConfig(\'' + encodeURIComponent(user.username) + '\')" title="Copy Config" class="p-1.5 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-md transition shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>' +
                                    '<button onclick="copyJsonConfig(\'' + encodeURIComponent(user.username) + '\')" title="Copy JSON" class="p-1.5 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-purple-50 dark:hover:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-md transition shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg></button>' +
                                    '<button onclick="showQR(\'' + encodeURIComponent(user.username) + '\')" title="QR Code" class="p-1.5 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-green-50 dark:hover:bg-green-900/30 text-green-600 dark:text-green-400 rounded-md transition shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg></button>' +
                                    '<button onclick="editUser(\'' + encodeURIComponent(user.username) + '\')" title="Edit" class="p-1.5 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-yellow-50 dark:hover:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 rounded-md transition shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>' +
                                    '<button onclick="deleteUser(\'' + encodeURIComponent(user.username) + '\')" title="Delete" class="p-1.5 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-red-50 dark:hover:bg-red-950/20 text-red-600 dark:text-red-400 rounded-md transition shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>' +
        ...
        function copySubLink(encodedUsername) {
            ...
            navigator.clipboard.writeText(getSubLink(username)).then(() => {
                alert('✅ Text subscription link copied!');
            }).catch(() => {
                alert('Error copying subscription link!');
            });
        }
        ...
        function copyStatusLink(encodedUsername) {
            ...
            navigator.clipboard.writeText(getStatusLink(username)).then(() => {
                alert('✅ Status page link copied!');
            }).catch(() => {
                alert('Error copying status link!');
            });
        }
        ...
        function copyJsonSubLink(encodedUsername) {
            ...
            navigator.clipboard.writeText(getJsonSubLink(username)).then(() => {
                alert('✅ JSON subscription link copied!');
            }).catch(() => {
                alert('Error copying JSON subscription link!');
            });
        }
        ...
        function showSubQR(encodedUsername, type) {
            const username = decodeURIComponent(encodedUsername);
            if (type === 'normal') {
                toggleQRModal(true, getSubLink(username), 'Text Sub QR');
            } else if (type === 'json') {
                toggleQRModal(true, getJsonSubLink(username), 'JSON Sub QR');
            }
        }
        ...
        function copyJsonConfig(encodedUsername) {
                        ...
                        outbounds: [
                            ...
                            },
                            {
                                ...
                                settings: {
                                    fragment: {
                                        packets: "tlshello",
                                        length: document.getElementById('frag-length').value || "20-30",
                                        interval: document.getElementById('frag-interval').value || "1-2"
                                    }
                                },
                                streamSettings: {
                                    sockopt: {
            ...
            navigator.clipboard.writeText(JSON.stringify(configArray, null, 2)).then(() => {
                ...
                alert('Error copying JSON config!');
            });
        }
        ...
        function showQR(encodedUsername) {
            ...
            const link = getVlessLink(username);
            if (!link) return;
            toggleQRModal(true, link, 'QR Code - ' + username);
        }
        ...
        async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/itzsepanta/RyxoPanel/refs/heads/main/ryxo.js?t=' + Date.now());
                ...
                if (latestVersion && latestVersion !== CURRENT_VERSION) {
                    ...
                    if (isManual) {
                        if (confirm('New version (v' + latestVersion + ') available! Would you like to update?')) {
                            applyUpdate();
                        }
                    }
                } else {
                ...
                if (isManual) {
                    document.getElementById('update-toggle').classList.remove('animate-pulse');
                    alert('Error checking for updates from GitHub.');
                }
            }
        }
        ...
        async function applyUpdate() {
            ...
            btn.disabled = true;
            alert('Fetching and applying update... Please confirm.');
            ...
            try {
                ...
                if (res.ok && data.success) {
                    alert('Panel updated successfully! Page will reload.');
                    window.location.reload();
                } else {
                    alert('Error updating panel: ' + (data.error || 'Unknown'));
                    btn.disabled = false;
                }
            } catch (err) {
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ryxo Panel - User Status</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:opsz@14..32&display=swap" rel="stylesheet" />
    <script>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">Ryxo Panel - User Status</h1>
            ...
            <div id="live-connections-badge" class="hidden inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-full text-xs font-bold shadow-sm">
                ...
                <span id="live-connections-text">0 devices connected</span>
            </div>
        </div>
        ...
        <div class="space-y-5 mb-8 relative z-10">
            ...
            <div class="bg-white/40 dark:bg-zinc-900/30 border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span id="req-pct" class="text-xs font-bold text-emerald-500">0%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-zinc-800 rounded-full h-2.5 overflow-hidden mb-3">
                    <div id="req-progress" class="bg-emerald-600 h-2.5 rounded-full transition-all duration-1000" style="width: 0%"></div>
                </div>
                <div class="flex justify-between text-xs text-gray-500 dark:text-zinc-400 font-medium">
                    <span>Used: <span id="used-req" class="font-bold text-gray-800 dark:text-zinc-200">-</span></span>
                    <span>Limit: <span id="limit-req" class="font-bold text-gray-800 dark:text-zinc-200">-</span></span>
                </div>
            </div>
            ...
            <div class="bg-white/40 dark:bg-zinc-900/30 border border-gray-200 dark:border-amoled-border rounded-2xl p-5 shadow-sm">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span id="online-pct" class="text-xs font-bold text-sky-500">0%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-zinc-800 rounded-full h-2.5 overflow-hidden mb-3">
                    <div id="online-progress" class="bg-sky-600 h-2.5 rounded-full transition-all duration-1000" style="width: 0%"></div>
                </div>
                <div class="flex justify-between text-xs text-gray-500 dark:text-zinc-400 font-medium">
                    <span>Online: <span id="online-count" class="font-bold text-gray-800 dark:text-zinc-200">0</span></span>
                    <span>Max: <span id="limit-online" class="font-bold text-gray-800 dark:text-zinc-200">-</span></span>
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-gray-100 dark:border-zinc-800 pt-6 relative z-10">
            ...
            <div class="space-y-3">
                <button onclick="copyTextSub()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-indigo-500 dark:hover:border-indigo-500 rounded-xl text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2">⛓️ Copy Text Subscription Link</span>
                    <span class="text-indigo-500">Copy</span>
                </button>
                <button onclick="copyVlessConfig()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-blue-500 dark:hover:border-blue-500 rounded-xl text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2">🚀 Copy VLESS Config</span>
                ...
                <button onclick="copyJsonSub()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-purple-500 dark:hover:border-purple-500 rounded-xl text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2">🌐 Copy JSON Subscription Link</span>
                    <span class="text-purple-500">Copy</span>
                </button>
                <button onclick="showQR()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-emerald-500 dark:hover:border-emerald-500 rounded-xl text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2">📱 Show Subscription QR Code</span>
                    <span class="text-emerald-500">Show</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4 mt-6 z-10">
        ...
        <a href="https://ryxo-deployer.sepantagamer724.workers.dev" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-emerald-500 dark:hover:text-emerald-400 group">
            <svg class="w-5 h-5 text-emerald-500 group-hover:scale-110 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            Deployer
        </a>
    </div>
    ...
    <div id="qr-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
        <div class="w-full max-w-sm bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
            <h3 class="font-bold text-gray-900 dark:text-zinc-100 mb-4">Scan QR Code</h3>
            <div class="bg-white p-3 rounded-xl inline-block mb-4 border border-gray-100">
                <div id="qrcode-box" class="flex justify-center items-center w-48 h-48 mx-auto"></div>
            </div>
            <button onclick="toggleQRModal(false)" class="w-full py-2 bg-gray-100 hover:bg-gray-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 font-medium rounded-lg text-sm transition text-gray-900 dark:text-zinc-100">Close</button>
    ...
    <script>
        ...
        function copyVlessConfig() {
            navigator.clipboard.writeText(getVlessLink()).then(() => alert('✅ VLESS config copied!'));
        }

        function copyJsonSub() {
            const link = window.location.protocol + '//' + getHost() + '/feed/json/' + encodeURIComponent(window.statusUser.username);
            navigator.clipboard.writeText(link).then(() => alert('✅ JSON subscription link copied!'));
        }

        function copyTextSub() {
            const link = window.location.protocol + '//' + getHost() + '/feed/' + encodeURIComponent(window.statusUser.username);
            navigator.clipboard.writeText(link).then(() => alert('✅ Text subscription link copied!'));
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (u.online_count && u.online_count > 0) {
                document.getElementById('live-connections-text').innerText = u.online_count + (u.max_connections ? '/' + u.max_connections : '') + ' devices connected';
                ...
                document.getElementById('live-connections-text').innerText = '0 devices connected';
            ...
            if (limitReq) {
                ...
                document.getElementById('limit-req').innerText = 'Unlimited';
                document.getElementById('req-pct').innerText = '0%';
            ...
            if (maxConns) {
                ...
                document.getElementById('limit-online').innerText = 'Unlimited';
                document.getElementById('online-pct').innerText = '0%';
            ...
            if (u.is_active === 0) {
                ...
                statusText.innerText = '❌ Status: Disabled / Manually Blocked';
                ...
                statusText.innerText = '⚠️ Status: Traffic Limit Exceeded';
            } else if (isReqExpired) {
                statusCard.className = 'mb-6 rounded-2xl p-4 text-center border font-bold relative z-10 bg-yellow-500/10 border-yellow-500/30 text-yellow-500 shadow-md shadow-yellow-500/5';
                statusText.innerText = '⚠️ Status: Request Limit Exceeded';
            } else if (isTimeExpired) {
                statusCard.className = 'mb-6 rounded-2xl p-4 text-center border font-bold relative z-10 bg-yellow-500/10 border-yellow-500/30 text-yellow-500 shadow-md shadow-yellow-500/5';
                statusText.innerText = '⏳ Status: Expired (Time Limit Reached)';
            } else {
                ...
                statusText.innerText = '✅ Status: Active & Connected';
            }
        });
    </script>
</body>
