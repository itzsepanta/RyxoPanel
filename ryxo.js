    ...
    if (url.pathname === "/api/update-panel" && request.method === "POST") {
      if (!env.CF_API_TOKEN || !env.CF_ACCOUNT_ID) {
        return new Response(
          JSON.stringify({
            error:
              "توکن یا اکانت آیدی کلودفلر تنظیم نشده است. لطفا با سایت زیر اپدیت کنید https://itzsepanta.ir/",
          }),
      ...
      try {
        ...
        if (!githubRes.ok)
          throw new Error("خطا در دریافت سورس جدید از گیت‌هاب");
        ...
        if (!bindingsData.success)
          throw new Error("عدم دسترسی به تنظیمات ورکر. توکن نامعتبر است.");
        const newBindings = [];
        ...
        if (!deployData.success)
          throw new Error("خطا در اعمال آپدیت در کلودفلر.");
        return new Response(JSON.stringify({ success: true }), {
        ...
        const errorMsg =
          err.message +
          " | در صورت عدم موفقیت، از طریق لینک زیر آپدیت کنید: https://itzsepanta.ir/";
        ...
        if (request.method === "PUT") {
          ...
          if (body.toggle_only !== undefined) {
            ...
            if (body.reset_action === "volume") {
              await env.DB.prepare(
                "UPDATE users SET used_gb = 0 WHERE username = ?",
              )
                .bind(username)
                .run();
              ...
              await env.DB.prepare(
                "UPDATE users SET used_req = 0 WHERE username = ?",
              )
                .bind(username)
                .run();
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
            ...
            <div class="space-y-3">
                ...
                <a href="https://itzsepanta.ir/" target="_blank" class="w-full py-3.5 bg-orange-500 hover:bg-orange-600 dark:bg-orange-600 dark:hover:bg-orange-500 text-white font-bold rounded-xl text-sm transition duration-300 flex items-center justify-center gap-2 border border-orange-400 dark:border-orange-500 shadow-sm">
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">پنل ریکسو - وضعیت اشتراک</h1>
